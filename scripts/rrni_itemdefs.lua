...
local itemNames = {
	-- Vanilla/DLC Weapons
	"item_tazer", "item_tazer_2", "item_tazer_3", "item_tazer_4",
	"item_power_tazer_1", "item_power_tazer_2", "item_power_tazer_3", "item_power_tazer_4", --T1 of this actually gets a "_1" for some reason
	"item_armor_tazer_1", "item_armor_tazer_1_17_15", "item_armor_tazer_2", "item_armor_tazer_3", "item_armor_tazer_4", --T1 of this actually gets a "_1" for some reason
	"item_dartgun", "item_dartgun_ammo", "item_bio_dartgun", --neural DART, RIOT DART, biogenic DART (this awful gun naming inconsistency)
	"item_dartgun_dam", "item_dartgun_ultra", --drilling dart, mono molecular rail gun
	"item_light_pistol", "item_light_pistol_KO", "item_light_pistol_ammo", --plasma gun, K&O pistol, hand cannon (p, pk, pa)
	"item_light_pistol_dam", "item_rifle_ultra", --cooker, K&O lance rifle
	
	-- Vanilla/DLC Items
	"item_stim", "item_stim_2", "item_stim_3", "item_stim_4",
	"item_paralyzer", "item_paralyzer_2", "item_paralyzer_3", "item_paralyzer_4",
	"item_portabledrive", "item_portabledrive_2","item_portabledrive_3",
	"item_icebreaker", "item_icebreaker_2" ,"item_icebreaker_3", "item_icebreaker_4",
	"item_emp_pack", "item_emp_pack_2", "item_emp_pack_3", "item_emp_pack_4",
	"item_shocktrap", "item_shocktrap_2", "item_shocktrap_3", "item_shocktrap_3_17_9",
	"item_laptop", "item_laptop_2", "item_laptop_3",
	"item_cloakingrig_1", "item_cloakingrig_2", "item_cloakingrig_3", "item_cloakingrig_3_17_5", --T1 of this actually gets a "_1" for some reason
	
	-- NIAA items
	"item_baton", "W93_weapon_baton_2", "W93_weapon_baton_3", "W93_weapon_baton_4",
	"W93_weapon_pwr_baton", "W93_weapon_pwr_baton_2", "W93_weapon_pwr_baton_3", "W93_weapon_pwr_baton_4",
}
...
local function getCategory (item)
	if item.abilities[ "installAugment" ] then return "AUGMENTS"
	elseif item.traits.slot == "melee" or item.traits.slot == "gun" then return "WEAPONS"
	else return "ITEMS"
	end
end
...
local detieredItemsByCategoryNIAA = {
	ITEMS = {
		...
		"item_shocktrap", "item_shocktrap_2", "item_shocktrap_3", "item_shocktrap_3_17_9",
		"item_laptop", "item_laptop_2", "item_laptop_3",
		"item_cloakingrig_1", "item_cloakingrig_2", "item_cloakingrig_3", "item_cloakingrig_3_17_5",
	},
	WEAPONS = {
		-- making sure my RANGED_TIERS option does not integrate with NIAA's unique guns
		"item_dartgun", "item_dartgun_ammo", "item_bio_dartgun",
		"item_dartgun_dam", "item_dartgun_ultra",
		"item_light_pistol", "item_light_pistol_KO", "item_light_pistol_ammo",
		"item_light_pistol_dam", "item_rifle_ultra",
	},
	AUGMENTS = {},
}
...
local function swapIcons( options )
	simlog("(RRNI Debug) Beginning to " .. (options.RRNI_ENABLED and "load" or "unload") .. " Roland's Roman Numeral Icons (RRNI) item icons...")
	...
	for _, itemName in ipairs(itemNames) do
		...
		if options.RRNI_ENABLED then
			...
			if not item then
				...
				if not RRNIIconName:match("_%d") then RRNIIconName = RRNIIconName .. "_1" end
				
				-- add ranged tweaks for icon names
				local isRanged = item.traits.weaponType and (item.traits.weaponType == "pistol" or item.traits.weaponType == "rifle")
				if isRanged then
					...
					if itemName:find("dartgun") and (itemName:find("dam") or itemName:find("ultra")) then
						local dartRifleFlag
						if options.RRNI_DART_RIFLE_ICON then dartRifleFlag = "_new" else dartRifleFlag = "_fix" end
					...
					if not options.RRNI_RANGED_TIERS then RRNIIconName = RRNIIconName:gsub("_%d","") end
				end
				
				-- generate final RRNI filenames (post-RANGED_TIERS check)
				local profile_100_filename = "gui/icons/item_icons/RRNI-icon-" .. RRNIIconName .. ".png"
				local profile_filename = "gui/icons/item_icons/items_icon_small/RRNI-icon-" .. RRNIIconName .. "_small.png"
	...
	if not ( skipCount == 0 ) then
		...
		if not ( unfoundCount == 0 ) then summary = summary .. (summary:find("%(",summary:len()) and " " or " / " ) .. unfoundCount .. " unfound itemdefs" end
		if not ( preloadCount == 0 ) then summary = summary .. (summary:find("%(",summary:len()) and " " or " / " ) .. preloadCount .. " preloaded icons" end
		if not ( NIAA_detiers == 0 ) then summary = summary .. (summary:find("%(",summary:len()) and " " or " / " ) .. NIAA_detiers .. " NIAA detiered items" end
		if not ( NIAA_skips == 0 ) then summary = summary .. (summary:find("%(",summary:len()) and " " or " / " ) .. NIAA_skips .. " disabled NIAA items" end
		summary = summary .. " )"
	end
		
	-- print final debug
	simlog("(RRNI Debug) DONE: " .. (options.RRNI_ENABLED and "Loaded" or "Unloaded") .. " icons for " .. swapCount .. " items" .. (summary and summary or ""))
end
