...
mui_tooltip_section.__concat = function(a, b)
    ...
    elseif type(a) == "string" and b.is_a and b:is_a(mui_tooltip_section) then
        ...
        assert(
                nil,
                "Invalid attempt to concat mui_tooltip_section between '" .. type(a) .. "' and '" ..
                        type(b) .. "'\n" .. debug.traceback())
    end
end
...
function mui_tooltip_section:activate(screen)
    ...
    if self._pseudoHotkeyTxt then
        ...
        local ctrlBinding = (self._qedctrl_pseudoBindingTxt and util.getControllerBindingImage and
                                    util.getControllerBindingImage(self._qedctrl_pseudoBindingTxt))
...
delayed_tooltip.__concat = function(a, b)
    if type(b) == "string" and a.is_a and a:is_a(delayed_tooltip) and a._primary_section and
            a._primary_section.__concat then
        a._primary_section = a._primary_section .. b
        return a
    elseif type(a) == "string" and b.is_a and b:is_a(delayed_tooltip) and b._primary_section and
            b._primary_section.__concat then
        b._primary_section = a .. b._primary_section
        return b
    else
        assert(
                nil,
                "Invalid attempt to concat delayed_tooltip between '" .. type(a) .. "' and '" ..
                        type(b) .. "'\n" .. debug.traceback())
    end
end
...
function abilityutil.uitr_wrappedOnTooltip(self, hud, sim, abilityOwner, abilityUser, ...)
    ...
    if not abilityUser then
        ...
        return self._uitr_oldOnTooltip and
                       self:_uitr_oldOnTooltip(hud, sim, abilityOwner, abilityUser, ...)
    elseif args[1] then
    ...
    if not tooltip or
            (not uitr_util.checkOption("overwatchAbilities") and not util.getControllerBindingImage) then
        return tooltip
    end
...
function abilityutil.uitr_wrappedAcquireTargets(
        self, targets, game, sim, abilityOwner, abilityUser, ...)
    ...
    -- UITR: Check for overwatch warnings.
    ...
    if uitr_util.checkOption("overwatchAbilities") then
        local triggersOverwatch = self.triggersOverwatch == true or
                                ...
                                sim, abilityOwner, abilityUser, ...))
        dangerousOverwatch = triggersOverwatch and abilityUser and
                                     simquery.isUnitUnderOverwatch(abilityUser)
    end

    -- UITR: Check for alternate target area modes.
    if target.setAreaMode and abilityOwner:getTraits().targetAreaMode ~= nil then
        target:setAreaMode(abilityOwner:getTraits().targetAreaMode)
    elseif target.setAreaMode then
        -- Check for cloud-generating items. Clouds use TARGETAREA_FLOOD_MANHATTAN by default.
        local grenadeTraits = abilityOwner:getTraits()
        if grenadeTraits.throwUnit then
            local template = unitdefs.lookupTemplate(grenadeTraits.throwUnit)
            assert(template, string.format("Could not find template '%s'", grenadeTraits.throwUnit))
            grenadeTraits = template.traits
        end
        if grenadeTraits.on_spawn and string.find(grenadeTraits.on_spawn, "cloud") then
            target:setAreaMode(simdefs.TARGETAREA_FLOOD_MANHATTAN)
        end
    end

    -- TODO: This only wraps targeters generated by abilities. Controller Bindings wants this
    -- wrapper on non-ability targeter tooltips as well. (Item targeting without a targeted ability
    -- doesn't set a target handler. In that case, onHudTooltip directly returns the cancel TT.
    -- Need to replace hud:init's assignment of the screen-level tooltip.)
